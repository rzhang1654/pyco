...
def connect(db):
  ...
  if db == 'sqlite':
    ...
    try:
      ...
      # if i export this it should go to mysql.connector but there is mis-match between the connector amd mysql server
      #import MySQLdb

      try:
        cxn = MySQLdb.connect (host = "10.107.31.109",
                             user = "root",
                             passwd = "minus361",
                             db = DBNAME)
      except:
...
def main():
  ...
  cxn = connect(db)
  if not cxn:
    printf('ERROR: %r not supported or unreachable, exiting' % db)
    return
  cur = cxn.cursor()
